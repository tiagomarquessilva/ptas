...
//  Assume-se que o primeiro param é o ponto de partida e o ultimo o ponto de chegada
...
try {
    ...
    $num_params = count($params->pontos);

    $ponto_partida = "(SELECT source AS node FROM rede_viaria
   ...
   LIMIT 1)";
    ...
    $ponto_chegada = "(SELECT source AS node FROM rede_viaria
   ORDER BY ST_Distance(
  ...
  LIMIT 1)";
    ...
    // Se menos de 3 pontos executa apenas pgr_dijkstra, se 3 ou mais entao pgr_tsp e depois pgr_dijkstra
    if ($num_params <= 1) {

        $ponto = pg_fetch_all(pg_query_params($db_connection, "SELECT id, ST_AsGeoJSON(localizacao) AS localizacao, (SELECT source
        FROM rede_viaria
        ORDER BY ST_Distance(ST_StartPoint(geom), pontos_arte_urbana.localizacao, true) ASC LIMIT 1) AS node
    FROM pontos_arte_urbana
    WHERE is_active = true AND id = $1", array($params->pontos[0])))[0];

        $rpp = pg_fetch_all(pg_query($ponto_partida));
        $rpc = pg_fetch_all(pg_query($ponto_chegada));
        //echo var_dump($ponto);
        $seq = array($rpp[0], $ponto, $rpc[0]);
        //echo var_dump($seq);
    } else {
        // Obter a sequencia de visita (pgr_tsp)
        $query_seq = $query_seq = "SELECT tsp.seq AS n, pau.id AS id, tsp.node AS node, ST_AsGeoJSON(pau.localizacao) AS localizacao
        ...
        FROM pontos_arte_urbana
        WHERE is_active = true AND id IN (".  implode(", ", $params->pontos) ."))),
                    directed := false
                )
                $$,
    ...
    for ($i2 = 0; $i2 <= ($num_results - 2); $i2++) {
        //echo var_dump($seq);
        //echo var_dump($i2);
        //echo var_dump(array($seq[$i2]["node"], $seq[$i2 + 1]["node"]));
